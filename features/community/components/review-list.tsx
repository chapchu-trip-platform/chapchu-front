...
function ReviewListContent() {
  ...
  return <div className="space-y-3 p-4">
    ...
    {placeId ? <>
      ...
      <PlaceReviews key={placeId} placeId={placeId} ownIds={new Set(mine.data?.map(review => review.id) ?? [])} />
    </> : <>
      <QueryFeedback loading={mine.loading} error={mine.error} onRetry={mine.reload} />
      {mine.data?.length === 0 && <p className="py-10 text-center text-[13px] text-warm-gray">아직 작성한 여행 리뷰가 없어요.</p>}
      {mine.data?.map(review => <ReviewCard key={review.id} review={review} own onPlace={() => setPlaceId(review.placeId)} onDeleted={() => mine.setData(previous => previous?.filter(item => item.id !== review.id) ?? null)} />)}
    </>}
  </div>
}

function PlaceReviews({ placeId, ownIds }: { placeId: string; ownIds: Set<string> }) {
  ...
  return <div className="space-y-3">
    ...
    {query.data?.map(review => <ReviewCard key={review.id} review={review} own={ownIds.has(review.id)} onDeleted={() => query.setData(previous => previous?.filter(item => item.id !== review.id) ?? null)} />)}
  </div>
}

function ReviewCard({ review, own, onPlace, onDeleted }: { review: Review; own: boolean; onPlace?: () => void; onDeleted: () => void }) {
  ...
  return <article className="space-y-3 rounded-card border border-border bg-card-surface p-4">
