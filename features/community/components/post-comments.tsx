...
export function PostComments({ postId, count, onCountChange, children }: { postId: string; count: number; onCountChange: (total: number) => void; children?: ReactNode }) {
  ...
  return <>
    ...
    {children}
    <motion.section id="community-comments" initial={prefersReducedMotion ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={transition} className="border-t border-border px-4 py-4">
    ...
    <motion.div layout={!prefersReducedMotion} className="space-y-4">
      {orderComments(comments).map(({ comment, depth }) => <motion.div layout={prefersReducedMotion ? false : 'position'} key={comment.id} initial={prefersReducedMotion ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={transition} style={{ marginLeft: Math.min(depth, 3) * 16 }} className={depth ? 'border-l border-border pl-3' : ''}>
        {!comment.deleted && <p className="text-[12px] text-warm-gray">{comment.nickname} · {formatCommunityDate(comment.createdAt)}</p>}
        <motion.p initial={prefersReducedMotion ? false : { opacity: 0 }} animate={{ opacity: 1 }} transition={transition} className="mt-1 whitespace-pre-wrap break-words text-[13px] text-deep-brown">{comment.deleted ? '삭제된 댓글입니다' : comment.content}</motion.p>
        ...
        {!comment.deleted && editing === comment.id && <motion.form key="edit" initial={prefersReducedMotion ? false : { height: 0, opacity: 0, y: -4 }} animate={{ height: 'auto', opacity: 1, y: 0 }} transition={transition} className="overflow-hidden space-y-2 rounded-xl bg-muted p-3" onSubmit={event => {
        ...
        {!comment.deleted && deleting === comment.id && <motion.div key="delete" initial={prefersReducedMotion ? false : { height: 0, opacity: 0, y: -4 }} animate={{ height: 'auto', opacity: 1, y: 0 }} transition={transition} className="overflow-hidden space-y-2 rounded-xl bg-muted p-3">
    ...
    <form className="mb-20 shrink-0 border-t border-border bg-card-surface px-4 py-3" onSubmit={event => { event.preventDefault(); submit() }}>
      {replyTo && <motion.div initial={prefersReducedMotion ? false : { height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} transition={transition} className="mb-2 flex items-center justify-between overflow-hidden text-[12px] text-sage-green"><span>답글 작성 중</span><Button size="sm" variant="ghost" disabled={busy} onClick={() => setReplyTo(null)}>답글 취소</Button></motion.div>}
      <div className="flex gap-2">
        ...
        <motion.div whileTap={prefersReducedMotion || !content.trim() || busy ? undefined : { scale: 0.9 }}><IconButton type="submit" aria-label="댓글 전송" size="lg" variant={content.trim() ? 'primary' : 'muted'} disabled={!content.trim() || busy}><Send className="h-4 w-4" /></IconButton></motion.div>
      </div>
    </form>
  </>
}
