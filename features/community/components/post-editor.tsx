    ...
    <div className="no-scrollbar flex-1 overflow-y-auto px-4 pb-28 pt-4">
      {preview ? <div ref={previewRef} tabIndex={-1} aria-label="게시글 미리보기" className="space-y-4 rounded-card border border-border bg-card-surface p-4 outline-none">
        ...
        <div className="space-y-2">
          ...
          <Input ref={titleRef} id="post-title" value={title} maxLength={POST_TITLE_LIMIT} placeholder="어떤 이야기를 나누고 싶으세요?" onChange={event => update({ title: event.target.value })} aria-describedby="post-title-count" />
        ...
        <div className="space-y-2">
          ...
          <textarea id="post-content" value={content} maxLength={POST_CONTENT_LIMIT} placeholder="반려동물과의 일상이나 궁금한 이야기를 자유롭게 적어 주세요." onChange={event => update({ content: event.target.value })} aria-describedby="post-content-count" className={`${communityTextAreaClass} min-h-64`} />
      ...
      <div className="mt-5 space-y-3 rounded-card bg-sage-green-light p-4">
        ...
        <div className="flex gap-2">
          <Button variant="outline" className="flex-1" disabled={!hasDraft} onClick={saveDraft}>임시 저장</Button>
