...
interface CommunityBoardProps { initialPostId?: string; initialTab?: 'free' | 'review' }
...
function Board({ initialPostId, initialTab }: CommunityBoardProps) {
  ...
  if (initialPostId) return <PostDetail key={initialPostId} postId={initialPostId} travelReview={initialTab === 'review'} onBack={() => router.back()} />
      ...
      <motion.div key={activeTab} initial={prefersReducedMotion ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={transition}>
        ...
        {activeTab === 2 ?
          <PostList
            ...
            onOpen={id => router.push(`/community?post=${encodeURIComponent(id)}&tab=review`)}
          /> : <PostList
          sort={activeTab === 0 ? 'popular' : 'latest'}
          category={activeTab === 1 ? 'FREE' : undefined}
          onOpen={id => router.push(`/community?post=${encodeURIComponent(id)}${activeTab === 1 ? '&tab=free' : ''}`)}
        />}
      </motion.div>
    </div>
  </motion.div>
