...
- 대상 화면: `/`, `/home`
- API 기준: [Chapchu API 문서](https://api.chapchu.site/docs/index.html) (문서 표기 최종 갱신 2026-08-18)
- 이번 문서 범위: 개발 전 설계 및 API 요청사항 정리. 기능 코드는 아직 변경하지 않는다.
...
4. 위치 권한은 사용 목적을 먼저 설명한 뒤 사용자 동작으로 요청한다.
...
- 현재 위치 지도, 주소, 날씨, 반려견, 주변 장소, HOT 게시글이 모두 mock 데이터다.
- Home 상단에 `NotificationButton`이 남아 있어 제외 요구사항과 다르다.
- 현재 Home은 로딩, 빈 결과, 일부 API 실패, 위치 거부 상태가 없다.
- TMAP SDK 로더와 지도 컴포넌트는 이미 존재하므로 새 지도 SDK를 추가하지 않고 재사용할 수 있다.
...
- `lib/api/endpoints.ts`에 문서의 `GET /home`, `GET /places/nearby`가 아직 없다.
- 현재 프론트의 커뮤니티 경로는 `/community/posts`이지만 문서의 실제 경로는 `/posts`다.
...
- Home 최초 렌더에서 바로 브라우저 권한 팝업을 띄우지 않는다.
- 위치 사용 목적을 먼저 보여주고 “현재 위치 사용”을 누른 직후 `getCurrentPosition()`을 호출한다.
...
> 내 주변의 반려동물 동반 장소와 날씨를 보여드리기 위해 현재 위치를 사용해요. 위치는 화면을 만드는 동안에만 사용하고 저장하지 않아요.
...
- 현재 지역은 수성구 고정 예시이며 브라우저 위치 권한을 요청하거나 사용자 좌표를 전송하지 않는다. 위치 동의 기능이 추가되면 고정값을 위·경도→격자 및 행정구역코드 변환 결과로 교체한다.
