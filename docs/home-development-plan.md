...
- 초기 Home/Map 설정 단계는 `watchPosition()`을 최대 12초만 사용해 여러 측정값 중 가장 작은 `accuracy`를 선택한다. 100m 이내면 즉시 종료하고, 최선 결과도 1km를 초과하면 지도·날씨 위치로 사용하지 않는다. 일시적인 측위 실패는 이 품질 측정 창을 즉시 끝내지 않으며, 화면 이탈·로그아웃·store 초기화 시 진행 중인 감시를 즉시 해제한다.
